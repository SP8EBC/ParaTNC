...
#define SPI_BUSY_DIFF_SLAVE \
	10 //!< Communication has been requested for different slave than what transmission is ongoing
	   //!< for
...
uint8_t spi_init_full_duplex_pio (spi_transfer_mode_t mode, spi_clock_polarity_strobe_t strobe,
								  int speed, int endianess);

//! Sends data asynchronously to selected SPI slave, discards anything what will come in over MISO
//! line
uint8_t spi_tx_data (uint32_t slave_id, uint8_t tx_from_internal, uint8_t *tx_buffer,
					 uint16_t ln_to_tx);

//! Sends data synchronously to selected slave and after that receives data from it. Everything
//! received before TX is finish is discarded.
uint8_t spi_rx_tx_data (uint32_t slave_id, uint8_t tx_from_internal, uint8_t *rx_buffer,
						uint8_t *tx_buffer, uint16_t ln_to_rx, uint16_t ln_to_tx);

//! Sends data to selected slave and in the same moment receive everything sent from slave. It
//! receive exactly ln_to_exchange bytes
uint8_t spi_rx_tx_exchange_data (uint32_t slave_id, uint8_t tx_from_internal, uint8_t *rx_buffer,
								 uint8_t *tx_buffer, uint16_t ln_to_exchange);
