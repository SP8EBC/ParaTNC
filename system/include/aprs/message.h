...
typedef struct message_t {
	...
	uint8_t content_ln;
	uint8_t number;
	uint8_t number_str[MESSAGE_NUMBER_STRING_BUFFER]; //!< Message sequence number but stored as
													  //!< string not decoded to integer
 ...
 * @param content_position optional position of an APRS message content (recipient callsign). if set
 * to zero function will look for it
 * @param src
 * @param output parsed APRS message content
 * @return zero if message has been found and decoded, non zero if parsing failed
 */
uint8_t message_decode (const uint8_t *const message, const uint16_t message_ln,
						uint16_t content_position, message_source_t src, message_t *output);
 ...
 * @return
 */
uint16_t message_encode (message_t *input, uint8_t *output, uint16_t output_ln,
						 message_source_t encode_for);
 ...
 */
uint8_t message_is_for_me (const char *const callsign, const uint8_t ssid,
 ...
 * @param message
 */
int message_create_ack_for (uint8_t *out_buffer, const uint16_t out_buffer_ln,
							const message_t *const message);
