...
static sim800_registration_status_t gsm_sim800_registration_status =
	SIM800_REGISTRATION_STATUS_UNKNOWN; // unknown
	...
	if (*state == SIM800_UNKNOWN) {
		...
		*state = SIM800_NOT_YET_COMM;
		...
		srl_send_data (srl_context,
					   (const uint8_t *)AUTOBAUD_STRING,
					   SRL_MODE_ZERO,
					   strlen (AUTOBAUD_STRING),
					   SRL_INTERNAL);
		...
		*state = SIM800_HANDSHAKING;
		...
		if (gsm_at_command_sent_last == 0) {
			...
			srl_send_data (srl_context,
						   (const uint8_t *)GET_PIN_STATUS,
						   SRL_MODE_ZERO,
						   strlen (GET_PIN_STATUS),
						   SRL_INTERNAL);
			...
			if (gsm_sim800_registration_delay_seconds > 0) {
				...
				// ask for network registration status
				srl_send_data (srl_context,
							   (const uint8_t *)GET_NETWORK_REGISTRATION,
							   SRL_MODE_ZERO,
							   strlen (GET_NETWORK_REGISTRATION),
							   SRL_INTERNAL);
			...
			// ask for network registration status
			srl_send_data (srl_context,
						   (const uint8_t *)GET_REGISTERED_NETWORK,
						   SRL_MODE_ZERO,
						   strlen (GET_REGISTERED_NETWORK),
						   SRL_INTERNAL);
			...
			srl_send_data (srl_context,
						   (const uint8_t *)GET_SIGNAL_LEVEL,
						   SRL_MODE_ZERO,
						   strlen (GET_SIGNAL_LEVEL),
						   SRL_INTERNAL);
			...
			if (gsm_sim800_simcard_status == SIMCARD_READY &&
				(gsm_sim800_network_status == NETWORK_REGISTERED ||
				 gsm_sim800_network_status == NETWORK_REGISTERED_ROAMING)) {
				*state = SIM800_ALIVE;
			}
			else {
		...
		if (gsm_at_command_sent_last == 0) {
			srl_send_data (srl_context,
						   (const uint8_t *)SHUTDOWN_GPRS,
						   SRL_MODE_ZERO,
						   strlen (SHUTDOWN_GPRS),
						   SRL_INTERNAL);
			...
			srl_send_data (srl_context,
						   (const uint8_t *)TRANSPARENT_MODE_ON,
						   SRL_MODE_ZERO,
						   strlen (TRANSPARENT_MODE_ON),
						   SRL_INTERNAL);
			...
			srl_send_data (srl_context,
						   (const uint8_t *)CONFIGURE_DTR,
						   SRL_MODE_ZERO,
						   strlen (CONFIGURE_DTR),
						   SRL_INTERNAL);
			...
			sim800_gprs_create_apn_config_str ((char *)srl_context->srl_tx_buf_pointer,
											   srl_context->srl_tx_buf_ln,
											   gsm_sim800_mcc_from_imsi,
											   gsm_sim800_mnc_from_imsi);

			// send created data to GSM module
			// srl_send_data(srl_context, srl_context->srl_tx_buf_pointer, SRL_MODE_ZERO,
			// strlen((const char *)srl_context->srl_tx_buf_pointer), SRL_EXTERNAL);
			...
			srl_send_data (srl_context,
						   (const uint8_t *)GET_IP_ADDRESS,
						   SRL_MODE_ZERO,
						   strlen (GET_IP_ADDRESS),
						   SRL_INTERNAL);
			...
			//			srl_send_data(srl_context, (const uint8_t*) ENABLE_EDGE, SRL_MODE_ZERO,
			// strlen(ENABLE_EDGE), SRL_INTERNAL);
			//
			//			// wait for command completion
			//			srl_wait_for_tx_completion(srl_context);
			//
			...
			//			gsm_waiting_for_command_response = 1;
			//
			//			srl_receive_data_with_callback(srl_context,
			// gsm_sim800_rx_terminating_callback);
			//
			...
			srl_send_data (srl_context,
						   (const uint8_t *)GET_CONNECTION_STATUS,
						   SRL_MODE_ZERO,
						   strlen (GET_CONNECTION_STATUS),
						   SRL_INTERNAL);
 ...
 */
uint8_t gsm_sim800_rx_terminating_callback (uint8_t current_data, const uint8_t *const rx_buffer,
											uint16_t rx_bytes_counter)
{
	...
	if (current != '\n' && current != '\r' && (before == '\n' || before == '\r') &&
		gsm_receive_newline_counter < 2) {
		gsm_response_start_idx = rx_bytes_counter;
	}

	// if an echo is enabled and second newline has been received
	if (gsm_at_comm_echo == 1 && gsm_receive_newline_counter > gsm_terminating_newline_counter &&
		gsm_response_start_idx > 0) {
...
void gsm_sim800_rx_done_event_handler (srl_context_t *srl_context, gsm_sim800_state_t *state)
{
	...
	if ((newlines & 0xFFFFFF00) != 0) {
		...
		if (second_line != 0) {
			gsm_sim800_check_for_async_messages (srl_context->srl_rx_buf_pointer +
													 gsm_response_start_idx + second_line,
												 srl_context->srl_rx_buf_ln,
												 &new_start_idx);
		}
		...
		if (third_line != 0) {
			gsm_sim800_check_for_async_messages (srl_context->srl_rx_buf_pointer +
													 gsm_response_start_idx + third_line,
												 srl_context->srl_rx_buf_ln,
												 &new_start_idx);
		}
		...
		if (fourth_line != 0) {
			gsm_sim800_check_for_async_messages (srl_context->srl_rx_buf_pointer +
													 gsm_response_start_idx + fourth_line,
												 srl_context->srl_rx_buf_ln,
												 &new_start_idx);
		}
	}
		...
		if (gsm_at_command_sent_last == GET_NETWORK_REGISTRATION) {
			...
			if (comparision_result == 0) {
				comparision_result = atoi (
					(const char *)(srl_context->srl_rx_buf_pointer + gsm_response_start_idx + 9));
			...
			sim800c_imsi_decode (gsm_sim800_imsi,
								 strlen (gsm_sim800_imsi),
								 &gsm_sim800_mcc_from_imsi,
								 &gsm_sim800_mnc_from_imsi);
			...
			if (comparision_result == 0) {
				strncpy (
					gsm_sim800_simcard_status_string,
					(const char *)(srl_context->srl_rx_buf_pointer + gsm_response_start_idx + 7),
					10);

				text_replace_non_printable_with_space (gsm_sim800_simcard_status_string,
													   SIM_STATUS_LENGHT);
			...
			if (comparision_result == 0) {
				...
				// return: SAT+COPS?\r\r\n+COPS: 0\r\n\r\nOK\r\n
				if (*(const char *)(srl_context->srl_rx_buf_pointer + gsm_response_start_idx + 7) ==
						'0' &&
					*(const char *)(srl_context->srl_rx_buf_pointer + gsm_response_start_idx + 9) !=
						'0') {
					...
					// copy network name from serial buffer into separate module, keep a room for
					// null terminator
					strncpy (gsm_sim800_registered_network,
							 (const char *)(srl_context->srl_rx_buf_pointer +
											gsm_response_start_idx + 12),
							 REGISTERED_NETWORK_LN - 1);

					// replace all non printable characters with space
					text_replace_non_printable_with_space (gsm_sim800_registered_network,
														   REGISTERED_NETWORK_LN);

					// trim network name with excessive spaces
					text_replace_space_with_null (gsm_sim800_registered_network,
												  REGISTERED_NETWORK_LN);
			...
			comparision_result =
				strncmp (SIGNAL_LEVEL,
						 (const char *)(srl_context->srl_rx_buf_pointer + gsm_response_start_idx),
						 5);

			if (comparision_result == 0) {
				comparision_result = atoi (
					(const char *)(srl_context->srl_rx_buf_pointer + gsm_response_start_idx + 6));
