 ...
 * @param config_gsm
 */
void sim800_gprs_initialize (srl_context_t *srl_context, gsm_sim800_state_t *state,
							 const config_data_gsm_t *config_gsm)
{
 ...
 * @param mnc
 */
void sim800_gprs_create_apn_config_str (char *buffer, uint16_t buffer_ln, const sim800_mcc_t mcc,
										const uint8_t mnc)
{
 ...
 */
void sim800_gprs_response_callback (srl_context_t *srl_context, gsm_sim800_state_t *state,
									uint16_t gsm_response_start_idx)
{
	...
	if (gsm_at_command_sent_last == SHUTDOWN_GPRS && srl_context->srl_rx_state != SRL_RX_ERROR) {
		comparision_result =
			strncmp (SHUTDOWN_GRPS_RESPONSE,
		...
		if (stringln > 8) {
			...
			for (i = stringln; i > 0; i--) {
				comparision_result =
					strncmp (STATE, (const char *)srl_context->srl_rx_buf_pointer + i, 7);

				if (comparision_result == 0) {
					memcpy (gsm_sim800_connection_status_str,
							srl_context->srl_rx_buf_pointer + i + 7,
							stringln - i - 7);
