...
void beacon_send_own(uint16_t voltage, uint8_t rtc_ok, uint8_t max_ok) {
	...
	if (voltage == 0 && rtc_ok == 0) {
		  ...
		  main_own_aprs_msg_len = sprintf(main_own_aprs_msg, "=%s%c%c%s%c%c %s [vbat: %d][rtc_ok: %d][max_ok: %d]", main_string_latitude, main_config_data_basic->n_or_s, main_symbol_f, main_string_longitude, main_config_data_basic->e_or_w, main_symbol_s, main_config_data_basic->comment, (int)voltage , (int)rtc_ok, (int)max_ok);
	}
