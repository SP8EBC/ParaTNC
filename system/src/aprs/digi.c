...
uint8_t digi_process (struct AX25Msg *msg, const config_data_basic_t *const config,
					  const config_data_mode_t *const config_mode)
{
	...
	if ((msg->src.ssid < 7 || msg->src.ssid > 9) &&
		(digi_mode == DIGI_ON_SSID_WIDE1 || digi_mode == DIGI_VISCOUS_SSID_WIDE1)) {
	...
	if (msg->rpt_cnt >= 1 && digi_msg_len == 0) {
		...
		//			if (msg->rpt_cnt == 1 && strcmp("WIDE2", msg->rpt_lst[0].call) == 0 &&
		//(msg->rpt_lst[0].ssid == 1 || msg->rpt_lst[0].ssid == 2)) {
		...
		//				sprintf(digi_path[2].call, "%s", "SR8WXO");	// zamiana WIDE2-2 albo WIDE2-1
		//na znak digi 				digi_path[2].ssid = 0x40; 				sprintf(digi_path[3].call, "%s", "WIDE2");	//
		//dodawanie WIDE2* 				digi_path[3].ssid = 0x40;			/* 0x40 oznacza jedynkę na 6 bicie
		//(przy numeracji od zera). Po przesunięciu o jedno miejsce 													   otrzymuje się 0x80 czyli jedynkę
		//na H-bicie */ 				digi_q = 1; 				call_len = 4;	// długość ścieżki

		//			}
		if (msg->rpt_cnt == 1 && strcmp ("WIDE1", msg->rpt_lst[0].call) == 0 &&
			msg->rpt_lst[0].ssid == 1) {
			/* Powtarzanie ścieżki WIDE1-1 */
			strcpy (digi_path[0].call, msg->dst.call); // znak docelowy
			digi_path[0].ssid = msg->dst.ssid;
			strcpy (digi_path[1].call, msg->src.call); // znak zrodlowy
			digi_path[1].ssid = msg->src.ssid;
			sprintf (digi_path[2].call,
					 "%s",
					 config->callsign); // zamiana WIDE2-2 albo WIDE2-1 na znak digi
			digi_path[2].ssid = (config->ssid | 0x40);
			sprintf (digi_path[3].call, "%s", "WIDE1"); // dodawanie WIDE2*
			digi_path[3].ssid =
				0x40; /* 0x40 oznacza jedynkę na 6 bicie (przy numeracji od zera). Po przesunięciu o
						 jedno miejsce otrzymuje się 0x80 czyli jedynkę na H-bicie */
			retval = DIGI_PACKET_DIGIPEATED;
			digi_call_len = 4; // długość ścieżki
		}
		else if (msg->rpt_cnt > 1 && strcmp ("WIDE1", msg->rpt_lst[0].call) == 0 &&
				 strcmp ("WIDE2", msg->rpt_lst[1].call) == 0 && msg->rpt_lst[0].ssid == 1 &&
				 msg->rpt_lst[1].ssid == 1) {
			...
			sprintf (digi_path[2].call,
					 "%s",
					 config->callsign); // zamiana WIDE1-1 na własny znak digi
			digi_path[2].ssid = (config->ssid | 0x40);
		...
		//			else if (msg->rpt_cnt >= 2 && strcmp("WIDE2", msg->rpt_lst[1].call) == 0 &&
		//msg->rpt_lst[1].ssid == 1) {
		...
		//				strcpy(digi_path[1].call, msg->src.call);	// znak zrodlowy
		//				digi_path[1].ssid = msg->src.ssid;
		//				sprintf(digi_path[2].call, "%s", msg->rpt_lst[0].call);	// dołączanie znaku
		//poprzedniego digi 				digi_path[2].ssid = (msg->rpt_lst[0].ssid | 0x40);
		//				sprintf(digi_path[3].call, "%s", "SR8WXO");	// zamiana WIDE2-1 na własny
		//znak digi 				digi_path[3].ssid = 0x40; 				sprintf(digi_path[4].call, "%s", "WIDE2");	//
		//dodawanie WIDE2* na końcu 				digi_path[4].ssid = 0x40; 				digi_q = 1; 				call_len = 5;
		//			}
		//			else if(msg->rpt_cnt >= 2 && strcmp("WIDE2", msg->rpt_lst[2].call) == 0 &&
		//(msg->rpt_lst[2].ssid == 1 || msg->rpt_lst[2].ssid == 2)) {
		...
		// 				sprintf(digi_path[3].call, "%s", "WIDE1");	// dodawanie WIDE1 na końcu
		//				digi_path[3].ssid = 0x40;
		//				sprintf(digi_path[4].call, "%s", "SR8WXO");		// zamiana WIDE2-1 albo
		//WIDE2-2 na własny znak digi 				digi_path[4].ssid = 0x40; 				sprintf(digi_path[5].call, "%s",
		//"WIDE2");	// dodawanie WIDE2* na końcu 				digi_path[5].ssid = 0x40; 				digi_q = 1; 				call_len = 6;
		//			}
		//			else if(msg->rpt_cnt >= 4 && strcmp("WIDE2", msg->rpt_lst[3].call) == 0 &&
		//(msg->rpt_lst[3].ssid == 1 || msg->rpt_lst[3].ssid == 2)) {
		...
		//				digi_path[1].ssid = msg->src.ssid;
		//				sprintf(digi_path[2].call, "%s", msg->rpt_lst[0].call);	// dołączanie znaku
		//poprzedniego digi WIDE1* 				digi_path[2].ssid = (msg->rpt_lst[0].ssid | 0x40);
		//				sprintf(digi_path[3].call, "%s", "WIDE1");	// dodawanie WIDE1
		//				digi_path[3].ssid = 0x40;
		//				sprintf(digi_path[4].call, "%s", msg->rpt_lst[2].call);	// dołączanie znaku
		//poprzedniego digi WIDE2* 				digi_path[4].ssid = (msg->rpt_lst[2].ssid | 0x40);
		//				sprintf(digi_path[5].call, "%s", "SR8WXO");		// zamiana WIDE2-1 na własny
		//znak digi 				digi_path[5].ssid = 0x40; 				sprintf(digi_path[6].call, "%s", "WIDE2");	//
		//dodawanie WIDE2* na końcu 				digi_path[6].ssid = 0x40; 				digi_q = 1; 				call_len = 7;
		//			}
		taskEXIT_CRITICAL ();
		...
		if (strncmp (msg->src.call, digi_path[1].call, 6) == 0) {
			...
			if (msg->src.ssid == digi_path[1].ssid) {
				...
				if (memcmp (msg->info, digi_msg, msg->len) == 0) {
					// if the message content is the same it means that current buffer content shall
					// be discarded and frame shall not be retransmited
					digi_msg_len = 0;
					memset (digi_msg, 0x00, CONFIG_AX25_FRAME_BUF_LEN);
