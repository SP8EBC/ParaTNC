...
#define SX1262_CHECK_NOK_RESULT(res, code) \
	if (res != SX1262_API_OK) {            \
		i = code;                          \
		goto nok;                          \
	}
...
const fanet_aircraft_stv_t fanet_stv = {.state = FANET_AIRCRAFT_STATUS_NEED_RIDE,
										.latitude = 49.7828f,
										.longitude = 19.0567f,
										.altitude = 512.0f,
										.speed = 1.0f,
 ...
 * 			0xFFFFFFFF 					if the call modem was idle and wait for busy condition
 * timeouted 0x80000000 < x < 0xFFFFFFFF if at the call modem was idle and then became busy for some
 * time
 */
...
int fanet_test (void)
{
	...
	fanet_wait_ret_history[fanet_api_it_history][0] =
		fanet_wait_not_busy (FANET_DLY_WRITE); // 0x22d8
	...
	////
	// 10. Configure DIO and IRQ: use the command SetDioIrqParams(...) to select TxDone IRQ and map
	// this IRQ to a DIO (DIO1,
	sx_result = sx1262_irq_dio_enable_disable_on_pin_dio1 (
		1,
		1,
		1,
		1); // command 0x08, 0x02, 0x43, 0x02 -- response 0xA2
	SX1262_CHECK_NOK_RESULT (sx_result, -13);
	////
	sx_result = sx1262_irq_dio_set_dio2_as_rf_switch (1); // command 0x9D, 0x01
	fanet_wait_ret_history[fanet_api_it_history][5] =
		fanet_wait_not_busy (FANET_DLY_WRITE); // 15 msec	0x7
	...
	if (SX1262_API_OK == sx_result && mode == SX1262_CHIP_MODE_STDBY_RC &&
		command_status == SX1262_LAST_COMMAND_RESERVED_OR_OK) {
		...
		fanet_wait_ret_history[fanet_api_it_history][8] =
			fanet_wait_not_busy (FANET_DLY_WRITE); // 0x2B
		////
		...
		if (type == SX1262_RF_PACKET_TYPE_LORA && sx_result == SX1262_API_OK) {
			...
			fanet_wait_ret_history[fanet_api_it_history][10] =
				fanet_wait_not_busy (FANET_DLY_WRITE); // 0x9a
			////
			sx_result = sx1262_status_get (&mode, &command_status);
			if (command_status == SX1262_LAST_COMMAND_RESERVED_OR_OK &&
				sx_result == SX1262_API_OK) {
				// 4. Define the Power Amplifier configuration with the command SetPaConfig(...)
				sx1262_modes_set_pa_config (5);
				fanet_wait_ret_history[fanet_api_it_history][12] =
					fanet_wait_not_busy (FANET_DLY_WRITE); // 0x20
				////
				...
				fanet_wait_ret_history[fanet_api_it_history][14] =
					fanet_wait_not_busy (FANET_DLY_WRITE); // 0x13
				////
				if (sx_result == SX1262_API_OK) {
					...
					fanet_wait_ret_history[fanet_api_it_history][15] =
						fanet_wait_not_busy (FANET_DLY_WRITE); // 0xD
					////
					// 7. Send the payload to the data buffer with the command WriteBuffer(...)
					sx1262_data_io_write_buffer (0,
												 FANET_SX_WAHT_TO_TRANSMIT_LN,
												 (const uint8_t *)FANET_SX_WHAT_TO_TRANSMIT);
					fanet_wait_ret_history[fanet_api_it_history][15] =
						fanet_wait_not_busy (FANET_DLY_WRITE); // 0xD
					////
					sx_result = sx1262_status_get (&mode, &command_status);
					if (command_status == SX1262_LAST_COMMAND_RESERVED_OR_OK &&
						sx_result == SX1262_API_OK) {
						// 8. Define the modulation parameter according to the chosen protocol with
						// the command SetModulationParams(...)1 These parameters are set using the
						// command SetModulationParams(...) which must be called after
						// SetPacketType(...).
						sx1262_rf_lora_modulation_params (SX1262_RF_LORA_SF7,
														  SX1262_RF_LORA_BW250,
														  SX1262_RF_LORA_CR_45,
														  SX1262_RF_LORA_OPTIMIZE_OFF);
						fanet_wait_ret_history[fanet_api_it_history][19] =
							fanet_wait_not_busy (FANET_DLY_WRITE); // 0x27
						////
						sx_result = sx1262_status_get (&mode, &command_status);
						if (command_status == SX1262_LAST_COMMAND_RESERVED_OR_OK &&
							sx_result == SX1262_API_OK) {
							// 9. Define the frame format to be used with the command
							// SetPacketParams(...)2
							sx1262_rf_lora_packet_params (5,
														  FANET_SX_WAHT_TO_TRANSMIT_LN,
														  SX1262_RF_LORA_HEADER_VARIABLE_LN_PACKET,
														  1,
														  0);
							fanet_wait_ret_history[fanet_api_it_history][21] =
								fanet_wait_not_busy (FANET_DLY_WRITE); // 0x27
							////
							sx_result = sx1262_status_get (&mode, &command_status);
							if (command_status == SX1262_LAST_COMMAND_RESERVED_OR_OK &&
								sx_result == SX1262_API_OK) {
								// 11. Define Sync Word value: use the command WriteReg(...) to
								// write the value of the register via direct register access
								sx1262_data_io_write_register_byte (0x740,
																	0xF4u); // Lora sync word MSB
								sx1262_data_io_write_register_byte (0x741,
																	0x14u); // Lora sync word LSB
								sx1262_data_io_read_register (
									0x740,
									1,
									(uint8_t *)&i); // read this sync word MSB back for verification
								sx1262_data_io_read_register (0x8E7, 1, (uint8_t *)&current_limit);
								////
								if (((uint8_t)(i & 0xFFu) == 0xF4u) &&
									((uint8_t)(current_limit & 0xFFu) == 0x38u)) {
									sx1262_data_io_read_register (
										0x741,
										1,
										(uint8_t
											 *)&i); // read this sync word LSB back for verification
									if ((uint8_t)(i & 0xFFu) == 0x14u) {
										// 12. Set the circuit in transmitter mode to start
										// transmission with the command SetTx(). Use the parameter
										// to enable
										const sx1262_api_return_t tx_res =
											sx1262_modes_set_tx (0x0); // TRANSMIT HERE
										fanet_wait_ret_history[fanet_api_it_history][28] =
											fanet_wait_not_busy (FANET_DLY_TRANSMIT); // 0x32eaf5
										////
										sx_result = sx1262_irq_dio_get_mask (
											&interrupt_mask); // command 0x12

										if (sx1262_is_interrrupt_flag_active () &&
											(interrupt_mask & 0x1) != 0) {
											fanet_tx_success_cnt++;
											i = 0;
											fanet_wait_ret_history[fanet_api_it_history]
																  [FANET_SX_WAIT_NUMBER - 1] =
																	  0x80000000;
										}
										else {
											fanet_wait_ret_history[fanet_api_it_history]
										...
										sx_result =
											sx1262_status_get (&mode,
															   &command_status); // command: 0xC0
										if (tx_res == SX1262_API_OK &&
											command_status == SX1262_LAST_COMMAND_TX_DONE) {
											i = 0;
										}
										else {
											i = -1;
										...
										sx_result = sx1262_irq_dio_get_mask (
											&last_interrupt_mask); // command 0x12

										sx1262_status_get_device_errors (&mode,
																		 &command_status,
																		 &errors);
									}
									else {
		...
		i = -2; // if (mode == SX1262_CHIP_MODE_STDBY_RC && command_status ==
				// SX1262_LAST_COMMAND_RESERVED_OR_OK)
	}
