...
/**
 * This is the restore voltage a battery must be charged to for ParaMETEO to restore it's normal
 * operation
 */
...
#define IWDG_STBY_STOP (0x3 << 17)
	...
	if ((option_byte_content & IWDG_STBY_STOP) != IWDG_STBY_STOP) {
		...
		// wait for any possible flash operation to finish (rather impossible here, but ST manual
	...
	case PWSAVE_NORMAL:
	case PWSAVE_AGGRESV: backup_reg_inhibit_periodic_pwr_switch (); break;
	}
}

/**
 * Entering STOP2 power save mode. In this mode all clocks except LSI and LSE are disabled.
 * StaticRAM content is preserved, optionally GPIO and few other peripherals can be kept power up
...
void pwr_save_exit_after_last_stop2_cycle (void)
{
	...
	uint16_t powersave_mode =
		backup_reg_get_powersave_state (); //(uint16_t)(REGISTER & ALL_STATES_BITMASK);
...
pwr_save_pooling_handler (const config_data_mode_t *config, uint8_t minutes_between_wx_frames,
						  ...
						  uint16_t vbatt_current, uint8_t *continue_loop)
{
	...
	*continue_loop = 1;
	...
	if (vbatt_current > PWR_SAVE_STARTUP_RESTORE_VOLTAGE_DEF) {
		...
		if (vbatt_current <= PWR_SAVE_CUTOFF_VOLTAGE_DEF &&
			vbatt_average <= (PWR_SAVE_CUTOFF_VOLTAGE_DEF + PWR_SAVE_CUTOFF_AVG_VOLTAGE_MARGIN)) {
			// if the battery voltage is below cutoff level and the ParaMETEO controller is
			// currently not cut off
	...
	if ((pwr_save_currently_cutoff & CURRENTLY_CUTOFF) != 0) {
		...
		// clear all previous powersave indication bits as we want to go sleep being already in L7
		// state
		pwr_save_clear_powersave_idication_bits ();

		// go sleep immediately and periodically check if battery has been charged above restore
		// level
	...
	const int sch_res =
		configuration_get_powersave_aggresive_schedule (&schedule_from, &schedule_to);
				...
				if (config->digi == 1) { // DIGI + WX
					...
					if (minutes_to_wx > WAKEUP_PERIOD_BEFORE_WX_FRAME_IN_MINUTES) {
						...
						pwr_save_switch_mode_to_l7 (
							...
							(WAKEUP_PERIOD_BEFORE_WX_FRAME_IN_MINUTES * 60));
		...
		if (config->wx != 0) {
			...
			if (config->gsm == 1) {
				...
				if (config->digi == 1) { // DIGI + WX + GSM
					...
					if (minutes_to_wx > WAKEUP_PERIOD_BEFORE_WX_FRAME_IN_MINUTES) {
						// if there is more than WAKEUP_PERIOD_BEFORE_WX_FRAME_IN_MINUTES minutes to
						// wx packet
						pwr_save_switch_mode_to_l7 (
							(wx_frame_interval * 60) -
							(WAKEUP_PERIOD_BEFORE_WX_FRAME_IN_MINUTES * 60)); // TODO: !!!
						...
						*continue_loop = 0;
						...
						// if there is WAKEUP_PERIOD_BEFORE_WX_FRAME_IN_MINUTES or less to next wx
						// packet
						reinit_sensors = pwr_save_switch_mode_to_c0 ();
					...
					if (minutes_to_wx > WAKEUP_PERIOD_BEFORE_WX_FRAME_IN_MINUTES) {
						...
						*continue_loop = 0;
						...
						if (pwr_save_seconds_to_wx <= 30) {
							...
							// do not reinitialize everything as reinitialization had been done when
							// switching to m4 mode
							reinit_sensors = 0;
						}
						else {
							if (config->powersave_keep_gsm_always_enabled == 0) {
