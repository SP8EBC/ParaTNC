...
uint8_t debug_hardfault_get_postmortem(debug_hardfault_postmortem_stackframe_t * out) {
    ...
    if (variant_validate_is_within_ram(out) != 0) {
        if (debug_hardfault_check_have_postmortem() != 0) {
            out->r0 = *((uint32_t*)(MEMORY_MAP_SRAM1_HFAULT_LOG_START) + DEBUG_HARDFAULT_OFFSET_R0);
            out->r1 = *((uint32_t*)(MEMORY_MAP_SRAM1_HFAULT_LOG_START) + DEBUG_HARDFAULT_OFFSET_R1);
            out->r2 = *((uint32_t*)(MEMORY_MAP_SRAM1_HFAULT_LOG_START) + DEBUG_HARDFAULT_OFFSET_R2);
            out->r3 = *((uint32_t*)(MEMORY_MAP_SRAM1_HFAULT_LOG_START) + DEBUG_HARDFAULT_OFFSET_R3);
            out->r12 = *((uint32_t*)(MEMORY_MAP_SRAM1_HFAULT_LOG_START) + DEBUG_HARDFAULT_OFFSET_R12);
            out->lr = *((uint32_t*)(MEMORY_MAP_SRAM1_HFAULT_LOG_START) + DEBUG_HARDFAULT_OFFSET_LR);
            out->pc = *((uint32_t*)(MEMORY_MAP_SRAM1_HFAULT_LOG_START) + DEBUG_HARDFAULT_OFFSET_PC);
            out->xpsr = *((uint32_t*)(MEMORY_MAP_SRAM1_HFAULT_LOG_START) + DEBUG_HARDFAULT_OFFSET_XPSR);
            out->source = *((uint32_t*)(MEMORY_MAP_SRAM1_HFAULT_LOG_START) + DEBUG_HARDFAULT_OFFSET_SOURCE);
            out->cfsr = *((uint32_t*)(MEMORY_MAP_SRAM1_HFAULT_LOG_START) + DEBUG_HARDFAULT_OFFSET_CFSR);
											   ...
											   char *output_string, uint16_t output_string_size)
{
	...
	case DEBUG_HARDFAULT_SOURCE_HFLT: {
		ouput_ln = snprintf (output_string, output_string_size,
							 "[!][HF][LR: 0x%lX][SP: 0x%lX][CFSR: 0x%lX][R0: 0x%lX][R1: 0x%lX][R2: "
							 "0x%lX][R3: 0x%lX]",
							 in->lr, in->pc, in->cfsr, in->r0, in->r1, in->r2, in->r3);
		break;
	}
