...
inline static int8_t wx_handler_temperature_check_slew (const float last,
														const float_average_t *average)
{
...
int32_t wx_get_temperature_measurement (const config_data_wx_sources_t *const config_sources,
										const config_data_mode_t *const config_mode,
										const config_data_umb_t *const config_umb,
										const config_data_rtu_t *const config_rtu, float *output)
{
	...
	// choose main temperature source from the configuration. main sensor is something which is used
	// to send data though aprs
	...
	case WX_SOURCE_INTERNAL_PT100: {
		// internal means sensors connected directly to the controller - one-wire and/or I2C on the
		// PCB it has nothing to do with distinction between external and internal temperature
		...
		if (config_mode->wx_ms5611_or_bme == 1) {
			...
			// ms5611 is a bit different as the sensor (internal) temperature is collected
			// separately from pressure
		...
		if ((measurement_result == BME280_OK) || (measurement_result == MS5611_OK)) {
			...
			event_log_sync (EVENT_WARNING,
							...
							0,
							0,
							(uint32_t)rte_wx_temperature_internal);
		}
		...
		if (configuration_get_disable_dallas () == 0) {
			...
			const uint8_t dallas_slew_exceeded =
				wx_handler_temperature_check_slew (dallas_temperature, &rte_wx_dallas_average);
		...
		if (configuration_get_disable_dallas () == 0) {
			...
			else if (config_sources->temperature == WX_SOURCE_INTERNAL &&
	...
	case WX_SOURCE_DAVIS_SERIAL: break;
	}
