...
const char *event_log_severity_to_str (event_log_severity_t severity)
{
	...
	case EVENT_ASSERT:
		return event_log_str_severity_assert; /**< EVENT_ASSERT assert failure, which result in hard
												 reset*/
	case EVENT_BOOTUP:
		return event_log_str_severity_bootup; /**< EVENT_BOOTUP all info events generated during
												 bootup */
	case EVENT_TIMESYNC:
								  ...
								  uint16_t output_ln)
{
	...
	if (exposed->severity == EVENT_TIMESYNC) {
		...
		case 1:
			first = (char *)event_log_str_hardfault_lr;
			second = (char *)event_log_str_hardfault_pc;
			break;
		case 2:
			first = (char *)event_log_str_hardfault_r0;
			second = (char *)event_log_str_hardfault_r1;
			break;
		case 3:
			first = (char *)event_log_str_hardfault_r2;
			second = (char *)event_log_str_hardfault_r3;
			break;
		case 4:
			first = (char *)event_log_str_hardfault_r12;
			second = (char *)event_log_str_hardfault_cfsr;
			break;
		case 5:
			first = (char *)event_log_str_hardfault_src;
			second = (char *)event_log_str_hardfault_xpsr;
			break;
		default:
			first = (char *)event_log_default;
			second = (char *)event_log_default;
			break;
		}

		// value of 5 contain hard fault source, which might be decoded from numeric value to
		// dedicated string
		if (exposed->param == 5) {
			...
			case DEBUG_HARDFAULT_SOURCE_USAGEFLT:
				source = (char *)event_log_str_hardfault_usage;
				break;
