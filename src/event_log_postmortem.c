	...
	if (have != 0) {
		...
		for (int i = 0; i < MEMORY_MAP_SRAM1_SUPERVISOR_LOG_32BWORDS_SIZE; i += 2) {
			...
			if (i == timestamp_idx) {
				...
				event_log_sync_triple (EVENT_ERROR,
									   EVENT_SRC_MAIN,
			...
			const supervisor_watchlist_t task =
				(supervisor_watchlist_t)((ptr[i] & 0xFF000000u) >> 24);
			...
			if (name_len <= 10) {
				...
				(name_len >= 3) ? _PUT_TASKNAME_INTO_WORD (wparam1, 2, 0)
								: _PUT_UNDERSCORE_INTO_WORD (wparam1, 0);
				(name_len >= 4) ? _PUT_TASKNAME_INTO_WORD (wparam1, 3, 1)
								: _PUT_UNDERSCORE_INTO_WORD (wparam1, 1);
				(name_len >= 5) ? _PUT_TASKNAME_INTO_WORD (wparam2, 4, 0)
								: _PUT_UNDERSCORE_INTO_WORD (wparam2, 0);
				(name_len >= 6) ? _PUT_TASKNAME_INTO_WORD (wparam2, 5, 1)
								: _PUT_UNDERSCORE_INTO_WORD (wparam2, 1);
				(name_len >= 7) ? _PUT_TASKNAME_INTO_WORD (lparam, 6, 0)
								: _PUT_UNDERSCORE_INTO_WORD (lparam, 0);
				(name_len >= 8) ? _PUT_TASKNAME_INTO_WORD (lparam, 7, 1)
								: _PUT_UNDERSCORE_INTO_WORD (lparam, 1);
				(name_len >= 9) ? _PUT_TASKNAME_INTO_WORD (lparam, 8, 2)
								: _PUT_UNDERSCORE_INTO_WORD (lparam, 2);
				(name_len >= 10) ? _PUT_TASKNAME_INTO_WORD (lparam, 9, 3)
								 : _PUT_UNDERSCORE_INTO_WORD (lparam, 3);
			}
			else {
