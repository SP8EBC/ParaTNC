...
typedef enum io_pool_vbat_r_state_t {
	...
	POOL_VBAT_R_DONT_SWITCH //!< more or less than 2 minutes to wx packet or VBAT_R was restarted
							//!< already
	...
	if (io_vbatt_state == IO_VBAT_ADC_DISABLE) {
		...
		if ((ADC2->ISR & ADC_ISR_EOC) == ADC_ISR_EOC) {
			if ((ADC2->ISR & ADC_ISR_EOS) == ADC_ISR_EOS) {
				...
				// adc has a resulution of 12bit, so with VDDA of 3.3V it gives about .00081V per
				// division
