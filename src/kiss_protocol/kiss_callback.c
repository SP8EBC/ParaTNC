 ...
 * @return
 */
int32_t kiss_callback_get_running_config (uint8_t *input_frame_from_host, uint16_t input_len,
...
int16_t kiss_pool_callback_get_running_config (uint8_t *output_buffer, uint16_t buffer_size)
{
	...
	if (kiss_async_message_counter * KISS_MAX_CONFIG_PAYLOAD_SIZE < conf_size) {
		...
		config_payload_size =
			conf_size - (kiss_async_message_counter * KISS_MAX_CONFIG_PAYLOAD_SIZE);
 ...
 * @return
 */
int32_t kiss_callback_get_version_id (uint8_t *input_frame_from_host, uint16_t input_len,
									  uint8_t *response_buffer, uint16_t buffer_size,
									  kiss_communication_transport_t transport_media)
{
	...
	config_payload_size =
		snprintf ((char *)response_buffer + 4, buffer_size, "METEO-%s-%s", SW_VER, SW_KISS_PROTO);
 ...
 * @return
 */
int32_t kiss_callback_erase_startup (uint8_t *input_frame_from_host, uint16_t input_len,
									 uint8_t *response_buffer, uint16_t buffer_size,
									 kiss_communication_transport_t transport_media)
{
 ...
 * @return
 */
int32_t kiss_callback_read_did (uint8_t *input_frame_from_host, uint16_t input_len,
								uint8_t *response_buffer, uint16_t buffer_size,
								kiss_communication_transport_t transport_media)
{
	 ...
	 *
	 * FEND, NONSTANDARD, RESPONSE_SIZE, KISS_READ_DID_RESP, DID_lsb, DID_msb, size_byte, DATA
	 * (...), FEND
	 */
 ...
 * @return
 */
int32_t kiss_callback_read_memory_by_addr (uint8_t *input_frame_from_host, uint16_t input_len,
										   uint8_t *response_buffer, uint16_t buffer_size,
										   kiss_communication_transport_t transport_media)
{
	/**
	...
	// address
	const uint32_t address = *(input_frame_from_host + 3) | (*(input_frame_from_host + 4) << 8) |
							 (*(input_frame_from_host + 5) << 16) |
							 (*(input_frame_from_host + 6) << 24);

	// size
	const uint8_t size = *(input_frame_from_host + 7);

	// if security access must be unlocked
	const uint8_t require_unlock =
		kiss_security_check_service_req_unlocking (KISS_READ_MEM_ADDR, transport_media, address);
	...
	if (size_format == 0x14) {
		...
		if (size > 0 && size < KISS_CONFIG_DIAGNOSTIC_BUFFER_LN - 4) {
			if ((require_unlock == 1 &&
				 kiss_security_access_get_unlocked_per_transport (transport_media) == 1) ||
				(variant_validate_is_within_read_mem_by_addr ((const void *)address) == 1)) {
				// construct DID response to an output buffer
				const uint8_t response_size =
					kiss_read_memory_response (address, size, response_buffer + 4, buffer_size - 5);
 ...
 * @return
 */
int32_t kiss_callback_reset (uint8_t *input_frame_from_host, uint16_t input_len,
							 uint8_t *response_buffer, uint16_t buffer_size,
							 kiss_communication_transport_t transport_media)
{
	...
	// if security access must be unlocked
	const uint8_t require_unlock =
		kiss_security_check_service_req_unlocking (KISS_RESTART, transport_media, reset_type);
 ...
 *
 */
int32_t kiss_callback_routine_control (uint8_t *input_frame_from_host, uint16_t input_len,
									   uint8_t *response_buffer, uint16_t buffer_size,
									   kiss_communication_transport_t transport_media)
{
	/**
	 ...
	 *
	 * FEND, KISS_ROUTINE_CONTROL, SUBFUNCTION_START_STOP_RESULT, ROUTINE_ID_LSB, ROUTINE_ID_MSB,
	 * lparam, wparam, FEND
	 */

	int32_t out = 0;

	// this is a result of starting or stopping a routine, but it might be also a result of routine
	// operation
	...
	const uint32_t lparam = *(input_frame_from_host + 5) | (*(input_frame_from_host + 6) << 8) |
							(*(input_frame_from_host + 7) << 16) |
							(*(input_frame_from_host + 8) << 24);
	...
	if (routine_type != 0) {
		...
		case KISS_ROUTINE_CONTROL_SUBFUNC_START:
			// greater than is used here instead of equals, because of a difference between
			// diagnostics got through serial port and decoded from aprs message. diagnostics from
			// aprs message has only FEND put at the beginning to retain offsets.
			if (input_len > KISS_CALLBACK_ROUTINE_CONTROL_START_MINIMUM_PAYLOAD_LN) {
				routine_processing_result =
					kiss_routine_control_start_routine (routineid, wparam, lparam);
				...
				if (routine_processing_result == KISS_ROUTINE_RETVAL_SUCCESSFULLY_STARTED) {
					...
					out =
						kiss_nrc_response_fill_incorrect_message_lenght_or_format (response_buffer);
				}
