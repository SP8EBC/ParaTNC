...
static int routine_5254_get_weekday_from_date (const LL_RTC_DateTypeDef *in)
{
	...
	const int out =
		(day + ((153 * (month + 12 * ((14 - month) / 12) - 3) + 2) / 5) +
		 ...
		 ((year + 4800 - ((14 - month) / 12)) / 100) + ((year + 4800 - ((14 - month) / 12)) / 400) -
		 32045) %
		7;
...
uint8_t routine_5254_start (uint32_t lparam, uint16_t wparam)
{
	...
	routine_5254_result |=
		(0x00FF & LL_RTC_DATE_Init (RTC, LL_RTC_FORMAT_BIN, &routine_5254_rtc_date));
	routine_5254_result |=
		(0xFF00 & LL_RTC_TIME_Init (RTC, LL_RTC_FORMAT_BIN, &routine_5254_rtc_time)) << 8;
