...
#define KISS_COMMUNICATION_APRSMSG_IS_DIGIT(c) \
 ...
 * @return
 */
kiss_communication_transport_t kiss_communication_aprsmsg_check_type (uint8_t *message_payload,
																	  uint16_t message_payload_ln)
{
...
/**
 *
 * @param message_payload pointer to characters buffer with APRS text message, containing encoded
 * UDS diag request
 * @param message_payload_ln length of buffer pointed by message_payload. processing will be done
 * until first non-hex character (non 0-9 / a-f) or this length
 * @param output_binary_buffer pointer to binary buffer where decoded data will be copied to.
 * @param output_ln
 * @return length of decoded UDS request or zero if message_payload doesn't contain valid hexstring
 * with UDS request
 */
uint16_t kiss_communication_aprsmsg_decode_hexstring (uint8_t *message_payload,
													  uint16_t message_payload_ln,
													  uint8_t *output_binary_buffer,
													  uint16_t output_ln)
{
	uint8_t out = 0;
	...
	if (variant_validate_is_within_ram (message_payload) == 1 && message_payload_ln > 0) {
		...
		if (KISS_COMMUNICATION_APRSMSG_DEC_CURRENT_CHAR == 'H' &&
			KISS_COMMUNICATION_APRSMSG_DEC_NEXT_CHAR == 'S') {
			...
			while (payload_it < message_payload_ln) {
				...
				if (KISS_COMMUNICATION_APRSMSG_IS_DIGIT (
						KISS_COMMUNICATION_APRSMSG_DEC_CURRENT_CHAR) == 0) {
				...
				// conveted byte
				const uint8_t converted_byte =
					(kiss_communication_aprsmsg_lookup_table[index_hn] * 0x10u) +
...
uint16_t kiss_communication_aprsmsg_encode_hexstring (uint8_t *input_binary_buffer,
													  uint16_t input_ln, uint8_t *output_message,
													  uint16_t output_message_max_ln)
{
	uint16_t out = 0;
	...
	// expected lenght of output buffer. Prefix 'HS', then hex encoded binary data and null
	// termimator at the end
	...
	if (output_message_max_ln > expected_output_ln) {
		...
		*(output_message + 1) = 'S';
		...
		while (iterator < input_ln) {
			...
			output_message[KISS_COMMUNICATION_APRSMSG_ENC_OUTPUT_L_IT (iterator)] =
				KISS_COMMUNICATION_APRSMSG_GET_CHAR (low_nibble);
			output_message[KISS_COMMUNICATION_APRSMSG_ENC_OUTPUT_H_IT (iterator)] =
				KISS_COMMUNICATION_APRSMSG_GET_CHAR (high_nibble);
