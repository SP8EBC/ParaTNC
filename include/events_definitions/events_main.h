/*
 * events_main.h
 *
 *  Created on: Jul 28, 2024
 *      Author: mateusz
 */
#ifndef EVENTS_DEFINITIONS_EVENTS_MAIN_H_
#define EVENTS_DEFINITIONS_EVENTS_MAIN_H_

/**
 * 	EVENT_BOOTUP	< EVENT_BOOTUP all info events generated during bootup
 *
 *
 */
#define EVENTS_MAIN_BOOTUP_COMPLETE 0xEEU

/**
 * 	EVENT_TIMESYNC	< EVENT_TIMESYNC event generated once at startup and then every 6 hours to
							   keep master_time and RTC date and time sync
 *
 *
 */
#define EVENTS_MAIN_TIMESYNC_BOOTUP	  0x77u
#define EVENTS_MAIN_TIMESYNC_PERIODIC 0x88u
#define EVENTS_MAIN_TIMESYNC_NTP	  0x99u

#define EVENTS_MAIN_CYCLIC 0xAAu

#define EVENTS_MAIN_ASSERT_FAIL 0xAEu

#define EVENTS_MAIN_CALLBACK_PRETX	0xB0u
#define EVENTS_MAIN_CALLBACK_POSTTX 0xB1u

#define EVENTS_MAIN_POSTMORTEM_HARDFAULT   0xF0u
#define EVENTS_MAIN_POSTMORTEM_SUPERVISOR  0xF1u
#define EVENTS_MAIN_CONFIG_FIRST_CRC_FAIL  0xF2u
#define EVENTS_MAIN_CONFIG_SECOND_CRC_FAIL 0xF3u
#define EVENTS_MAIN_CONFIG_FIRST_RESTORE   0xF4u
#define EVENTS_MAIN_CONFIG_SECOND_RESTORE  0xF4u

#define EVENTS_MAIN_ERROR_TASK_HANDLE_NULL 0xFAu

#endif
