...
/**
 * Important warning for ParaMETEO target!! Starting from EA15 software the configuration is kept in
 * separate NvMem area, outside application software area. It isn't erased or reprogrammed during
 * loading HEX file generated with this build. After NvMem is initialized, changing only this
 * configuration won't affect ParaMETEO. To force update flash mass erase shall be performed.
 */
...
// #define _MODBUS_RTU		// use Modbus RTU slave devices as a external meteo data source. For
// more configuration
...
#define _INTERNAL_AS_BACKUP // if defined ParaTNC will switch to internal sensors in case of
							// the communication with UMB/Dallas Serial/Modbus external sensors will
							// hang up
...
// #define _MUTE_RF	// TODO: Not yet implemented - This will make station RXonly and disable all
// data transmission #define _MUTE_OWN	// TODO: Not yet implemented - This will disable all
// self-generated packets (wx, telemetry, beacon)
...
#define _LONWE 'E'
#define _COMMENT \
	"WARNING! Put Your configuration in this file and then rename it to 'station_config.h'"

// You can use only one of these below defines to choose symbol. Meteo data are are always
// transmitted with blue WX symbol
// #define _SYMBOL_DIGI			// uncomment if you want digi symbol(green star with D inside)
#define _SYMBOL_WIDE1_DIGI // uncomment if you want 'little' digi symbol (green star with digit 1
						   // overlaid)
...
// Or you can keep commented all symbol defines and choose custom one based on data from APRS
// symbols table
// #define _SYMBOL_F	'/'
// #define _SYMBOL_S	'#'
...
// Comment this to disable beacon auto sending during startup (this can be risky if RF feedback
// occur)
...
// #define _RANDOM_DELAY	// adds random delay TO fixed time set by _DELAY_BASE. This additional
// time can be
...
// After waiting time declared above ParaTNC will check DCD (Data Carrier Detect) flag, which works
// as some kind of semaphore. If radio channel is not occupied by any other transmission TX will be
// keyed up immediately, otherwise software will wait for clear conditions.

// Few IMPORTANT hints about setting transmit delay properly.
//
// Transmit delay is key parameter to maintain RF network free from packet losses and collisions. If
// your station will be installed on tall object, without any other digi's close to it, you can set
// _DELAY_BASE to very low value and disable _RANDOM_DELAY. If you wanna rather auxiliary station,
// witch should only fill gap in RF coverage in small area, then _DELAY_BASE parameter should be not
// less than 12 (600msec), the smallest range the higher _DELAY_BASE should be. Additionally for
// gapfillers (auxiliary stations) _RANDOM_DELAY schould be enabled.
//
// This delay will ensure that while other station will be transmitting repeated packets from
// mobile, Yours will keep always quiet and won't jam RF network. This greatly improve DCD based
// access to channel. Various controllers uses various lenght of preamble, some of them produce
// signal which might be impossible to decode by ParaTNC, so DCD is only one part of effective
// multiaccess to medium.
...
// Do not touch this
#if defined(_SYMBOL_DIGI) && !defined(_SYMBOL_WIDE1_DIGI) && !defined(_SYMBOL_HOUSE) && \
	!defined(_SYMOL_RXIGATE) && !defined(_SYMBOL_IGATE)
#define _SYMBOL_F '/'
#define _SYMBOL_S '#'
...
#define _SYMBOL_S '&'
#elif !defined(_SYMBOL_DIGI) && !defined(_SYMBOL_WIDE1_DIGI) && !defined(_SYMBOL_HOUSE) && \
	!defined(_SYMOL_RXIGATE) && !defined(_SYMBOL_IGATE) && defined(_SYMBOL_SAILBOAT)
#define _SYMBOL_F '/'
#define _SYMBOL_S 'Y'
...
#if defined(PARATNC_HWREV_A) && (defined(_METEO) || defined(_DALLAS_AS_TELEM)) && \
	!defined(_DALLAS_SPLIT_PIN)
#define _DALLAS_SPLIT_PIN
#endif

#if defined(PARATNC_HWREV_B) && (defined(_METEO) || defined(_DALLAS_AS_TELEM)) && \
	!defined(_DALLAS_SPLIT_PIN)
#define _DALLAS_SPLIT_PIN
#endif
...
#if !defined(_ANEMOMETER_TX20) && !defined(_ANEMOMETER_ANALOGUE) && !defined(_UMB_MASTER) && \
	defined(_METEO)
