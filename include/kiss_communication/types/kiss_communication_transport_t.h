...
typedef enum kiss_communication_aprsmsg_transport_t {
	 ...
	 *      - 06FF11223344550000000 (...) - KISS message padded with zeros to 58 characters / 29
	 * bytes
	 *      - 6D6E - four characters / 2 bytes of salt
	 *
	 *    da1044bfccd776eb8b717ec3f945a4b799616a3bf4bf5ba08ba1f2d39301e5f0 - encrypted with
	 * AES128-ECB
	 *
	 *    Pmnda1044bfccd776eb8b717ec3f945a4b799616a3bf4bf5ba08ba1f2d39301e5f0 - encrypted &
	 * transported
	 *      - P - constant prefix
	 *      - 'mn' - two characters of salt
	 *      - 'da1044bf (...)' ebcrypted message
	 *
	 *  Internally the controller holds salt value from last UDS request in RTC backup register as a
	 * 16bit word. Each request is checked and a salt value cannot be smaller than the previous one,
	 * the check is done by 'converting' two character into one uint16_t value in way presented
	 * below" 'mn' - 'm' 0x6D, 'n' 0x6E => 0x6D6E Value of the salt could be reverted only if it is
	 * bigger than 0x7A00 ('~' as a first char), but it may be reverted only if  0x2100 <
	 * new_salt_value < 0x2200. In another words: reverted salt must have '!' as a first character.
	 *
	 *  Because UDS requests and responses are transmitted as APRS text message salt must contain
	 * only printable characters (of course except tab and space)
	 *
